    ...
    <div class="main-content">
                ...
                <div class="row g-3">
                    <div class="col-6">
                        <div class="card-modern">
                            ...
                            <div class="metric-value" id="suhuValue">{{ $currentData['suhu'] ?? 0 }}<small>°C</small>
                            </div>
                        </div>
                    </div>
                    <div class="col-6">
                        <div class="card-modern">
                            ...
                            <div class="metric-value" id="kelembapanValue">
                                {{ $currentData['kelembapan'] ?? 0 }}<small>%</small></div>
                        </div>
                    </div>
                    <div class="col-6">
                        <div class="card-modern">
                            <div class="metric-title"><i class="bi bi-cloud-haze2"></i> CO₂ (ppm)</div>
                            <div class="metric-value" id="co2Value">{{ $currentData['co2'] ?? 0 }}<small>ppm</small>
                            </div>
                        </div>
                    </div>
                    <div class="col-6">
                ...
                <div class="card-modern d-flex flex-column justify-content-between">
                    <div>
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            <h4 class="mb-0 fw-bold">{{ $activeBatch ?? 'Tidak Ada Batch Aktif' }} - Status Overview
                            </h4>

                            <span id="batchStatusBadge"
                                class="badge bg-white text-dark border px-3 py-2 rounded-pill shadow-sm">
                                Status: {{ ucfirst($batchInfo['status'] ?? 'None') }} <i
                        ...
                        <div class="mb-3">
                            <div class="d-flex justify-content-between align-items-center mb-1">
                                <span class="fw-semibold text-muted small">AI Prediction Progress (Kematangan): <span
                                        id="kematanganValue"
                                        class="text-success fw-bold">{{ $currentData['kematangan_pct'] ?? 0 }}
                                        %</span></span>
                    ...
                    <div class="border-top pt-3 d-flex gap-2 flex-wrap">
                        ...
                        @elseif (in_array($batchInfo['status'] ?? '', ['completed', 'cancelled']))
                            ...
                            <span
                                class="badge d-inline-flex align-items-center bg-light text-dark border px-3 rounded-3">
            ...
            <div class="col-xl-8 col-lg-7">
                <div class="card-modern">
                    <div class="d-flex justify-content-between mb-3">
                        <h5 class="fw-bold mb-0"><i class="bi bi-graph-up text-muted"></i> Progress Lintasan Parameter
                            : Suhu & CO₂</h5>
                ...
                <div class="card-modern d-flex flex-column justify-content-between">
                    <div>
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            <h5 class="fw-bold mb-0"><i class="bi bi-sliders text-muted"></i> Device Control & Status
                            </h5>
                        ...
                        <div class="border rounded-4 p-3 mb-3 bg-white shadow-sm">
                            ...
                            <div class="d-flex gap-3 text-muted small">
                                <div class="form-check"><input class="form-check-input" type="radio"
                                        name="bMode" id="bM" checked><label class="form-check-label"
                                        for="bM">Manual</label></div>
                                <div class="form-check"><input class="form-check-input" type="radio"
                                        name="bMode" id="bA"><label class="form-check-label"
                                        for="bA">Auto</label></div>
                            </div>
                        </div>
                        ...
                        <div class="border rounded-4 p-3 bg-white shadow-sm">
                            ...
                            <div class="d-flex gap-3 text-muted small">
                                <div class="form-check"><input class="form-check-input" type="radio"
                                        name="eMode" id="eM" checked><label class="form-check-label"
                                        for="eM">Manual</label></div>
                                <div class="form-check"><input class="form-check-input" type="radio"
                                        name="eMode" id="eA"><label class="form-check-label"
                                        for="eA">Auto</label></div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="border-top pt-2 mt-2 text-muted small d-flex justify-content-between">
                        ...
                        <div>Interval: <span class="fw-bold text-dark">{{ $system['simulation_interval'] ?? 0 }}
                                Detik</span></div>
                    </div>
                </div>
            </div>
        ...
        <div class="row g-3">
            <div class="col-12">
                <div class="card-modern">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        <h5 class="fw-bold mb-0"><i class="bi bi-layout-text-window-reverse text-muted"></i> Parameter
                            Data Tabel Log</h5>
                    ...
                    <div class="table-responsive">
                        <table class="table table-custom">
                            ...
                            <tbody id="tableBody">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="8" class="text-center text-muted">Belum ada data riwayat
                                            kompos untuk batch ini.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
    ...
    <script>
        ...
        async function refreshDashboard() {
            ...
            try {
                ...
                const response = await fetch('/dashboard-data?' + Date.now(), {
                    cache: "no-store"
                });
                ...
                // Dashboard Card
                // =========================
                document.getElementById('suhuValue').innerHTML =
                    `${current.suhu}<small>°C</small>`;

                document.getElementById('kelembapanValue').innerHTML =
                ...
                // Status Batch
                // =========================
                document.getElementById('timestampValue').innerHTML =
                    current.timestamp ?? '-';

                document.getElementById('hariValue').innerHTML =
                    current.hari ?? '-';

                document.getElementById('faseValue').innerHTML =
                    current.fase ?? '-';

                document.getElementById('kematanganValue').innerHTML =
                    (current.kematangan_pct ?? 0) + ' %';

                document.getElementById('sisaHariValue').innerHTML =
                    current.sisa_hari ?? '-';

                // =========================
                // Progress Bar
                // =========================
                const bar = document.getElementById('kematanganBar');

                bar.style.width = (current.kematangan_pct ?? 0) + "%";

                // =========================
                // Device
                // =========================
                document.getElementById('kipasValue').innerHTML =
                    current.kipas == 1 ? "ON" : "OFF";

                document.getElementById('kipasValue').className =
                    current.kipas == 1 ?
                    "badge bg-success text-white small" :
                    "badge bg-light text-dark border small";

                document.getElementById('blowerToggle').checked =
                    current.kipas == 1;

                document.getElementById('pengadukValue').innerHTML =
                    current.pengaduk == 1 ? "ON" : "OFF";

                document.getElementById('pengadukValue').className =
                    current.pengaduk == 1 ?
                    "badge bg-success text-white small" :
                    "badge bg-light text-dark border small";

                document.getElementById('em4Toggle').checked =
                    current.pengaduk == 1;
                ...
                document.getElementById('predictionStatus').className =
                    current.prediction_status == "completed" ?
                ...
                // Current Row
                // =========================
                document.getElementById('currentRowValue').innerHTML =
                    system.current_row ?? '-';
        ...
        async function refreshCharts() {
            ...
            try {
                ...
                const response = await fetch('/chart-data?' + Date.now(), {
                    cache: "no-store"
                });
